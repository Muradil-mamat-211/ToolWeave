...
def _strict_max_token_partitions(seqlen_list: List[int], max_token_len: int) -> List[List[int]]:
    """Best-fit-decreasing partitions with a true per-partition token cap.

    The legacy balanced partitioner derives a partition count from
    ``ceil(total / max_token_len)`` and then balances the total load.  That is
    useful for throughput, but an individual partition can exceed the value
    named ``max_token_len``.  This deterministic helper is used only when a
    caller explicitly requests a hard memory-safety cap.

    Every input sequence remains intact.  No padding, truncation, or token
    splitting is introduced.
    """

    if max_token_len <= 0:
        raise ValueError(f"max_token_len must be positive, got {max_token_len}")
    if any(length > max_token_len for length in seqlen_list):
        largest = max(seqlen_list)
        raise ValueError(
            "A single trajectory exceeds the strict token cap: "
            f"largest={largest}, max_token_len={max_token_len}"
        )

    partitions: List[List[int]] = []
    partition_sums: List[int] = []
    # Descending lengths improve packing; the original index is a stable
    # deterministic tie-breaker.
    for index in sorted(range(len(seqlen_list)), key=lambda i: (-seqlen_list[i], i)):
        length = seqlen_list[index]
        fitting = [
            bin_index
            for bin_index, current in enumerate(partition_sums)
            if current + length <= max_token_len
        ]
        if not fitting:
        ...
        # Best fit leaves the least residual capacity.  Lower bin index is the
        # deterministic tie-breaker.
        bin_index = min(
            fitting,
            key=lambda candidate: (
                max_token_len - partition_sums[candidate] - length,
                candidate,
            ),
        )
        partitions[bin_index].append(index)
        partition_sums[bin_index] += length

    assert all(total <= max_token_len for total in partition_sums)
    return partitions
...
def rearrange_micro_batches(
    ...
    Args:
        ...
        enforce_max_token_len (bool): when true, preserve complete trajectories
            while guaranteeing that each returned micro-batch has at most
            ``max_token_len`` effective tokens.
    ...
    if enforce_max_token_len:
        micro_bsz_idx = _strict_max_token_partitions(seq_len_effective, max_token_len)

        # Keep the existing cross-rank/virtual-pipeline count contract by
        # splitting already-safe bins when another rank needs more bins.
        target_num_micro_batches = len(micro_bsz_idx)
        if min_num_micro_batch is not None:
            target_num_micro_batches = max(target_num_micro_batches, min_num_micro_batch)
        if dist.is_initialized() and same_micro_num_in_dp:
            synchronized = torch.tensor([target_num_micro_batches], device=get_device_name())
            dist.all_reduce(synchronized, op=dist.ReduceOp.MAX, group=dp_group)
            target_num_micro_batches = synchronized.cpu().item()
        if num_batches_divided_by is not None:
            target_num_micro_batches = roundup_divisible(
                target_num_micro_batches, num_batches_divided_by
            )
        if target_num_micro_batches > len(seq_len_effective):
            raise ValueError(
                "Cannot create more non-empty strict micro-batches than trajectories: "
                f"target={target_num_micro_batches}, trajectories={len(seq_len_effective)}"
            )

        while len(micro_bsz_idx) < target_num_micro_batches:
            splittable = [
                (sum(seq_len_effective[index] for index in partition), position)
                for position, partition in enumerate(micro_bsz_idx)
                if len(partition) > 1
            ]
            if not splittable:
                raise ValueError("Unable to satisfy strict synchronized micro-batch count")
            _, position = max(splittable, key=lambda item: (item[0], -item[1]))
            moved_index = micro_bsz_idx[position].pop()
            micro_bsz_idx.append([moved_index])

        micro_batches = []
        for partition in micro_bsz_idx:
            curr_micro_batch = torch.cat([batch[index : index + 1] for index in partition])
            effective_tokens = int(curr_micro_batch["attention_mask"].sum().item())
            assert effective_tokens <= max_token_len, (
                f"strict token cap violated: {effective_tokens} > {max_token_len}"
            )
