...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        smoke_update_id = int(data.meta_info.get("smoke_global_step", -1))
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                if has_multi_modal_inputs:
                    ...
                    if self.config.use_dynamic_bsz:
                        ...
                        rearranged_text_micro_batches_tds, textual_indices = rearrange_micro_batches(
                            ...
                            enforce_max_token_len=self.config.get(
                                "ppo_enforce_max_token_len", False
                            ),
                        )
                ...
                for micro_batch_idx, data in enumerate(micro_batches):
                    ...
                    smoke_root = os.environ.get("RODS_SMOKE_ARTIFACT_DIR")
                    if smoke_root:
                        # Smoke-only, detached policy diagnostics. Saving the
                        # exact pre-update forward values does not alter PPO;
                        # it makes every ratio/KL/mask auditable afterward.
                        update_id = smoke_update_id
                        rank = torch.distributed.get_rank()
                        output_dir = Path(smoke_root) / "training" / f"update_{update_id}" / "actor_microbatches"
                        ...
                        payload = {
                            ...
                            "micro_batch_index": micro_batch_idx,
                            "responses": responses.detach().cpu(),
                            "response_mask": response_mask.detach().cpu(),
                            "old_log_probs": old_log_prob.detach().cpu(),
                            "current_log_probs": log_prob.detach().cpu(),
                            "ppo_ratio": torch.exp(log_prob.detach() - old_log_prob.detach()).cpu(),
                            "advantages": advantages.detach().cpu(),
                            "ref_log_probs": (
                                ref_log_prob.detach().cpu()
                                if self.config.use_kl_loss
                                else None
                            ),
                            "kl_penalty": (
                                kld.detach().cpu() if self.config.use_kl_loss else None
                            ),
                            "entropy": entropy.detach().cpu() if entropy is not None else None,
                            "pg_loss": float(pg_loss.detach().item()),
                            "policy_loss": float(policy_loss.detach().item()),
                            "clip_fraction": float(pg_clipfrac.detach().item()),
                        }
                        target = output_dir / (
                            f"rank_{rank}_epoch_{epoch}_mini_{batch_idx}_micro_{micro_batch_idx}.pt"
                        )
