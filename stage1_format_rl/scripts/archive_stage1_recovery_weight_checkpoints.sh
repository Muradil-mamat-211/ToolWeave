...
OLD_ARCHIVE="$WORKSPACE/outputs/stage1_format_qwen3_4b/weight_checkpoints"
...
LOG_DIR="$WORKSPACE/stage1_format_rl/logs/recovery_from_step25"
...
archive_step() {
    ...
    mapfile -t shards < <(find "$source" -maxdepth 1 -type f -name 'model_world_size_2_rank_*.pt' | sort)
    [[ "${#shards[@]}" -eq 2 && -s "${shards[0]}" && -s "${shards[1]}" ]] || return 1
    ...
    RUN_STEP="$run_step" LOGICAL_STEP="$logical_step" TARGET="$staging" SOURCE="$source" python - <<'PY'
import json
import os
...
payload = {
    ...
    "base_checkpoint": "/root/autodl-tmp/rods-workspace/outputs/stage1_format_qwen3_4b/weight_checkpoints/global_step_25",
    "model_start_path": "/root/autodl-tmp/rods-workspace/stage1_format_rl/artifacts/checkpoint_eval/merged/global_step_25",
