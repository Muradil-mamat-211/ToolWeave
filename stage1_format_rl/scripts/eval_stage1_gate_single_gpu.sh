...
MODEL_PATH="${1:?usage: eval_stage1_gate_single_gpu.sh MODEL_PATH LABEL VAL_FILE}"
LABEL="${2:?usage: eval_stage1_gate_single_gpu.sh MODEL_PATH LABEL VAL_FILE}"
VAL_FILE="${3:?usage: eval_stage1_gate_single_gpu.sh MODEL_PATH LABEL VAL_FILE}"
...
TMP_ROOT="$TOOLWEAVE_SHORT_TEMP_ROOT/stage1-gate-single/$LABEL"
...
printf '%s\n' "$CUDA_VISIBLE_DEVICES" > "$EVAL_DIR/gpu_id.txt"
