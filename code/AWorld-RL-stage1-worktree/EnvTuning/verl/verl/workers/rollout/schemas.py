...
class AsyncRolloutRequest(BaseModel):
    ...
    # Prompt-level BFCL context and per-action metadata are non-tensor rollout
    # provenance.  Actor spans are captured at token assembly time.
    matchtir_context: Dict[str, Any] = Field(default_factory=dict)
    assistant_token_spans: List[Dict[str, int]] = Field(default_factory=list)
    matchtir_policy_steps: List[Dict[str, Any]] = Field(default_factory=list)
    ...
    def add_assistant_message(
        ...
        actor_token_start = len(self.input_ids)
        self._update_input_ids(content_ids, attention_mask=True, loss_mask=True)
        self.assistant_token_spans.append(
            {
                "start": actor_token_start,
                "end": len(self.input_ids),
            }
        )

    def add_matchtir_policy_step(self, metadata: Dict[str, Any]) -> None:
        """Associate parser metadata with the most recent assistant token span."""

        step = dict(metadata)
        if not self.assistant_token_spans:
            step["actor_span_reliable"] = False
            step["provenance_reliable"] = False
            step["provenance_error"] = "assistant token span is missing"
            step["actor_token_start_absolute"] = -1
            step["actor_token_end_absolute"] = -1
        else:
            span = self.assistant_token_spans[-1]
            step["actor_span_reliable"] = True
            step["actor_token_start_absolute"] = int(span["start"])
            step["actor_token_end_absolute"] = int(span["end"])
        step["rollout_id"] = self.request_id
        step["rollout_offset"] = int(self.rollout_offset)
        self.matchtir_policy_steps.append(step)
