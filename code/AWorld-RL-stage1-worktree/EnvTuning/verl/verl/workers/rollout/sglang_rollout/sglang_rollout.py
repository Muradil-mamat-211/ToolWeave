...
try:
    # SGLang >=0.4.8; keep the legacy import for the repository's original
    # 0.4.6.post5 runtime. This compatibility form is used by veRL v0.5.0.
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        # veRL PR #2823 compatibility: DictConfig may omit engine_kwargs.
        # The smoke config selects Triton explicitly for Blackwell rather than
        # relying on the historical FA3 default used by this older fork.
        engine_kwargs = self.config.get("engine_kwargs", {}).get("sglang", {})
        attention_backend = engine_kwargs.get("attention_backend", None)
        custom_op_backend = engine_kwargs.get("custom_op_backend", None)
        if custom_op_backend not in (None, "cuda", "native"):
            raise ValueError(
                "engine_kwargs.sglang.custom_op_backend must be one of "
                "null, 'cuda', or 'native'"
            )
        if custom_op_backend == "native":
            # The process-level patch is also applied by smoke_instrumentation/
            # sitecustomize.py in SGLang's spawned scheduler subprocess.
            os.environ["RODS_SGLANG_FORCE_NATIVE_CUSTOM_OPS"] = "1"
            from sglang.srt.custom_op import CustomOp

            CustomOp.dispatch_forward = lambda self: self.forward_native

        optional_server_args = {}
        for key in (
            "sampling_backend",
            "disable_cuda_graph",
            "cuda_graph_max_bs",
            ...
            if key in engine_kwargs and engine_kwargs[key] is not None:
                optional_server_args[key] = engine_kwargs[key]
        optional_server_args.setdefault(
            "disable_cuda_graph", bool(self.config.get("enforce_eager", False))
        )
        optional_server_args.setdefault("context_length", self.config.max_model_len)
        ...
        **kwargs,
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                step_provenance = metrics.get("rods_matchtir_v1_step") if isinstance(metrics, dict) else None
                if isinstance(step_provenance, dict):
                    _req.add_matchtir_policy_step(step_provenance)
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        reward_scores = []
        matchtir_provenance = []
        from env_tuning.rods_matchtir_v1.provenance import response_relative_step, to_builtin
        ...
        for req in sorted_output_req_list:
            ...
            provenance = dict(req.matchtir_context)
            provenance.update(
                {
                    "rollout_id": req.request_id,
                    "rollout_offset": int(req.rollout_offset),
                    "batch_data_id": int(req.batch_data_id),
                    "policy_steps": [
                        response_relative_step(
                            step,
                            prompt_length=len(req.prompt_ids),
                            response_length=len(req.response_ids),
                        )
                        for step in req.matchtir_policy_steps
                    ],
                }
            )
            matchtir_provenance.append(to_builtin(provenance))
        ...
        return DataProto(
            ...
            non_tensor_batch={
                ...
                "matchtir_provenance": np.array(matchtir_provenance, dtype=object),
            },
        )
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int) -> list[AsyncRolloutRequest]:
        ...
        from env_tuning.rods_matchtir_v1.provenance import build_rollout_context

        req_list = []
        multi_modal_data_list = prompts.non_tensor_batch.get("multi_modal_data", [None] * len(prompts.non_tensor_batch["raw_prompt"]))
        data_source_list = prompts.non_tensor_batch.get(
            "data_source", [""] * len(prompts.non_tensor_batch["raw_prompt"])
        )
        for data_idx, (raw_prompt, multi_modal_data) in enumerate(zip(prompts.non_tensor_batch["raw_prompt"], multi_modal_data_list)):
            for rollout_offset in range(n):
                ...
                raw_messages = raw_prompt.tolist()
                matchtir_context = build_rollout_context(
                    messages=raw_messages,
                    interaction_kwargs=_interaction_kwargs,
                    data_source=str(data_source_list[data_idx]),
                )
                ...
                req = AsyncRolloutRequest(
                    ...
                    interaction_kwargs=_interaction_kwargs,
                    matchtir_context=matchtir_context,
