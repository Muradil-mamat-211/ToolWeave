...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # A combined actor/rollout/reference worker must track reference
        # offload independently from actor offload.  ``_is_offload_param`` is
        # intentionally actor-scoped in that role.
        self._is_ref_offload_param = self._is_ref and self.config.ref.fsdp_config.get(
            "param_offload", False
        )
        self._is_ref_phase_offload = self._is_ref_offload_param and self.config.ref.fsdp_config.get(
            "phase_param_offload", False
        )
        if self._is_actor:
    ...
    def _build_model_optimizer(
        ...
        # Respect the role-specific FSDP config for both actor and reference.
        # Historically this path forced every reference model onto CPU even
        # when ref.fsdp_config.param_offload=false, making a GPU-resident
        # reference impossible and contradicting the public config contract.
        # FSDP1 native CPUOffload retains an unsharded reference allocation
        # after forward on a world-size-one NO_SHARD worker.  The optional
        # phase mode instead uses the existing veRL whole-model helpers around
        # the reference forward, which guarantees that PPO starts without the
        # reference parameters resident.  Standard configurations retain the
        # native behavior.
        use_phase_param_offload = role == "ref" and fsdp_config.get(
            "phase_param_offload", False
        )
        cpu_offload = (
            CPUOffload(offload_params=True)
            if fsdp_config.get("param_offload", False) and not use_phase_param_offload
            else None
        )
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            if self._is_ref_phase_offload:
                offload_fsdp_model_to_cpu(self.ref_module_fsdp)
                log_gpu_memory_usage(
                    "After phase-offload ref model during init", logger=logger
                )
    ...
    def update_actor(self, data: DataProto):
        ...
        # Reclaim no-grad old/reference-logprob cache blocks before the
        # activation-heavy PPO forward/backward phase.  This releases only
        # unused allocator cache; live parameters, optimizer state, data, and
        # all training semantics remain unchanged.  It is particularly useful
        # for the single-GPU hybrid engine where these phases share a process.
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if self._is_ref_phase_offload:
            ...
            log_gpu_memory_usage(
                "After phase-load ref model during compute_ref_log_prob", logger=logger
            )
        ...
        try:
            ...
            if self._is_ref_phase_offload:
                # Release references to the GPU input before moving the model;
                # the returned reference log-prob tensor is already on CPU.
                del data
                offload_fsdp_model_to_cpu(self.ref_module_fsdp)
                log_gpu_memory_usage(
                    "After phase-offload ref model during compute_ref_log_prob",
                    logger=logger,
                )
